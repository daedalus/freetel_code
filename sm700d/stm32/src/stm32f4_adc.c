 ...
 *
 * Pin PA1 connects to ADC1, pin PA2 connects to ADC2.
 */
...
int adc1_read(int16_t buf[], int n) {
    return fifo_read(adc1_fifo, buf, n);
}

/* n signed 16 bit samples in buf[] if return != -1 */

int adc2_read(int16_t buf[], int n) {
...
void DMA2_Stream0_IRQHandler() {
    int i, j;
    int16_t signed_buf1[ADC_BUF_SZ / 2];
    int16_t signed_buf2[ADC_BUF_SZ / 2];
    int16_t sam;
    ...
    if(DMA_GetITStatus(DMA2_Stream0, DMA_IT_HTIF0) != RESET) {
        ...
        for(i=0, j=0; i<ADC_BUF_SZ/2; i+=2,j++) {
            sam = (int16_t) (adc_buf[i] - 32768);
            signed_buf1[j] = sam;

            sam = (int16_t) (adc_buf[i+1] - 32768);
            signed_buf2[j] = sam;
        }
    ...
    if(DMA_GetITStatus(DMA2_Stream0, DMA_IT_TCIF0) != RESET) {
        ...
        for(i=0, j=0; i<ADC_BUF_SZ/2; i+=2,j++) {
            sam = (int16_t) (adc_buf[ADC_BUF_SZ / 2 + i] - 32768);
            signed_buf1[j] = sam;

            sam = (int16_t) (adc_buf[ADC_BUF_SZ / 2 + i+1] - 32768);
            signed_buf2[j] = sam;
        }
