...
#define TONE_PART_SINE_LEN	(sizeof(partial_sine) / sizeof(partial_sine[0]))
 ...
 */
void tone_reset(struct tone_gen_t *const tone_gen, uint16_t freq, uint16_t duration) {
	if (freq)
		/* Compute the time step */
		tone_gen->step = (((2*freq*TONE_SINE_LEN) << TONE_SHIFT) / ((2*TONE_FS) + 1) + 1);
	else
		/* DC tone == silence */
		tone_gen->step = 0;

	/* Compute remaining samples */
	tone_gen->remain = (uint16_t)(((uint32_t)(TONE_FS * duration)) / 1000);
 ...
 */
int16_t tone_next(struct tone_gen_t *const tone_gen) {
	if (!tone_gen)
		return 0;
	...
	uint16_t sample_int = ((tone_gen->sample) >> TONE_SHIFT) % TONE_SINE_LEN;
