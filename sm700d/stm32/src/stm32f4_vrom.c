...
static const struct vrom_sector_idx_t *vrom_get_sector_hdr(uint8_t sector) {
	return (const struct vrom_sector_idx_t *)(VROM_START_ADDR + (VROM_SECT_SZ * sector));
}

/*!
 * Return the address of a virtual EEPROM block.
 */
static const struct vrom_data_block_t *vrom_get_block(uint8_t sector, uint8_t block) {
	return (const struct vrom_data_block_t *)((void*)vrom_get_sector_hdr(sector) + (VROM_BLOCK_SZ * (block + 1)));
}

/*!
 * Compute the CRC32 of a block.
 */
static uint32_t vrom_crc32(const struct vrom_data_block_t *const block) {
...
static const struct vrom_data_block_t *vrom_find(uint8_t rom, uint8_t idx) {
	...
	for (sector = 0; sector < VROM_SECT_CNT; sector++) {
		const struct vrom_sector_idx_t *sect_hdr = vrom_get_sector_hdr(sector);
...
static uint8_t vrom_block_num(const void *address) {
	...
	uint32_t offset = (uint32_t)(address - (const void *) vrom_get_sector_hdr(sector));
...
static int vrom_format_sector(const struct vrom_sector_idx_t *sector) {
	...
	if (FLASH_ProgramWord((uint32_t)sector, cycles_remain) == FLASH_COMPLETE)
		return 0;	/* All good */
 ...
 */
static int vrom_set_flags(const struct vrom_data_block_t *block, uint16_t flags) {
	const struct vrom_sector_idx_t *sector = vrom_get_sector_hdr(vrom_sector_num(block));
 ...
 * Write a new block.
 */
static int vrom_write_block(uint8_t rom, uint8_t idx, uint8_t size, const uint8_t *in) {
	...
	while(rem) {
		if (*out != *in) {
			if (FLASH_ProgramByte((uint32_t)out, *in) != FLASH_COMPLETE)
				/* Failed! */
				return -EIO;
		}
 ...
 */
static int vrom_rewrite_block(const struct vrom_data_block_t *block, uint8_t size, const uint8_t *in) {
	uint8_t obsolete = 0;
 ...
 */
static int vrom_overwrite_block(const struct vrom_data_block_t *block, uint8_t offset, uint8_t size, const uint8_t *in) {
	uint8_t data[VROM_DATA_SZ];
	...
	if (offset) {
		...
		memcpy(data, in, size);
		memcpy(&data[size], &(block->data[size]), block_sz - size);
	}
 ...
 */
static int vrom_write_internal(uint8_t rom, uint16_t offset, uint16_t size, const uint8_t *in) {
	...
	if (!block) {
		...
		memcpy(&data[block_offset], in, block_sz-block_offset);
		...
		/* Overwrite block */
		int res = vrom_overwrite_block(block, block_offset, block_sz, in);
	...
	while(size) {
		...
		if (block)
			res = vrom_overwrite_block( block, 0, block_sz, in);
		else
			res = vrom_write_block(rom, block_idx, block_sz, in);
 ...
 */
int vrom_write(uint8_t rom, uint16_t offset, uint16_t size, const void *in) {
	int res;
