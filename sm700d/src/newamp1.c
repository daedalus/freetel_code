...
float ftomel(float fHz) {
    float mel = floorf(2595.0f*log10f(1.0 + fHz/700.0f)+0.5f);
    return mel;
}
...
void mel_sample_freqs_kHz(float rate_K_sample_freqs_kHz[], int K, float mel_start, float mel_end)
{
    ...
    mel = mel_start;
    for (k=0; k<K; k++) {
        rate_K_sample_freqs_kHz[k] = 0.7f*(powf(10.0f, (mel/2595.0f)) - 1.0f);
        mel += step;
    }
}
...
void resample_const_rate_f(C2CONST *c2const, MODEL *model, float rate_K_vec[], float rate_K_sample_freqs_kHz[], int K)
{
    ...
    for(m=1; m<=model->L; m++) {
        ...
        rate_L_sample_freqs_kHz[m] = m*model->Wo*(c2const->Fs/2000.0f)/M_PI;
...
void interp_Wo_v(float Wo_[], int L_[], int voicing_[], float Wo1, float Wo2, int voicing1, int voicing2)
{
    ...
    if (voicing1 && voicing2) {
        float c;
        for(i=0,c=1.0; i<M; i++,c-=1.0f/M) {
            Wo_[i] = Wo1*c + Wo2*(1.0f-c);
            voicing_[i] = 1;
        }
    }
...
void determine_phase(C2CONST *c2const, COMP H[], MODEL *model, int Nfft, codec2_fft_cfg fwd_cfg, codec2_fft_cfg inv_cfg)
{
    ...
    for(m=1; m<=model->L; m++) {
        ...
        rate_L_sample_freqs_kHz[m] = (float)m*model->Wo*(c2const->Fs/2000.0f)/M_PI;        
    }
    
    for(i=0; i<Ns; i++) {
        sample_freqs_kHz[i] = (c2const->Fs/1000.0f)*(float)i/Nfft;
    }
    ...
    for(m=1; m<=model->L; m++) {
        b = floorf(0.5f+m*model->Wo*Nfft/(2.0f*M_PI));
...
void newamp1_interpolate(float interpolated_surface_[], float left_vec[], float right_vec[], int K)
{
    ...
    for(i=0,c=1.0; i<M; i++,c-=1.0f/M) {
        for(k=0; k<K; k++) {
            interpolated_surface_[i*K+k] = left_vec[k]*c + right_vec[k]*(1.0f-c);
        }
    }
}
                                   ...
                                   int    indexes[])
{
    ...
    post_filter_newamp1(rate_K_vec_no_mean_, rate_K_sample_freqs_kHz, K, 1.5f);
