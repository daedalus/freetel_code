...
static int extract_output(char out_char[], int DecodedBits[], int ParityCheckCount[], 
...
static float correction(float xinput )
{
  if (xinput > 2.625f )
    ...
    return( -0.375f * xinput + 0.6825f );
  else 
    return( -0.1875f * xinput + 0.5f );

}

static float LambdaAPPstar(float mag1, float mag2) {
  if (mag1 > mag2)
    return( fabsf( mag2 + correction( mag1 + mag2 ) - correction( mag1 - mag2 ) ) );
  else
    return( fabsf( mag1 + correction( mag1 + mag2 ) - correction( mag2 - mag1 ) ) );
}
...
static int extract_output(char out_char[], int DecodedBits[], int ParityCheckCount[], int max_iter, int CodeLength, int NumberParityBits) {
    int i, j;
